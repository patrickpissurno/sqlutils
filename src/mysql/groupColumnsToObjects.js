...
module.exports = function (rows, primary_key, groups) {
  ...
  for (let row of rows) {
    ...
    for (let group of groups) {
      ...
      if (map[id][group.out][row[group.foreign_key]] !== undefined) continue;

      if (row[group.foreign_key] == null) continue;

      let obj = {};
      for (let column of group.columns) obj[column.out] = row[column.name];

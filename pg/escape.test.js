...
tap.equal(
  escape(Buffer.from([255, 255, 0, 0])),
  `'\\xffff0000'`,
  'buffer works',
);
...
tap.equal(
  escape(`whomst'd've`),
  `'whomst''d''ve'`,
  'postgres custom string escaping',
);
...
tap.equal(
  escape(`' OR 1=1 --`),
  `''' OR 1=1 --'`,
  'sql injection 101 should not work',
);
tap.equal(
  escape('\fmore text'),
  `'\fmore text'`,
  '\\f should be escaped the right way (1)',
);
tap.equal(
  escape('more\ftext'),
  `'more\ftext'`,
  '\\f should be escaped the right way (2)',
);
tap.equal(
  escape('more text\f'),
  `'more text\f'`,
  '\\f should be escaped the right way (3)',
);
